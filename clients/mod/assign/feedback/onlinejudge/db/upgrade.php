...
function xmldb_assignfeedback_onlinejudge_upgrade($oldversion = 0) {
    ...
    if ($oldversion < 2011060700) {
        ...
        if ($rs->valid()) {
            foreach ($rs as $testcase) {
                ...
                if ($file) {
                    $filerecord = array('contextid' => $dstcontext->id, 'component' => 'mod_assignment', 'filearea' => 'onlinejudge_input', 'itemid' => $testcase->id, 'filepath' => '/', 'filename' => basename($testcase->inputfile));
                    $fs->create_file_from_storedfile($filerecord, $file);
                }

                $file = $fs->get_file($srccontext->id, 'course', 'legacy', 0, '/' . dirname($testcase->outputfile) . '/', basename($testcase->outputfile));
                if ($file) {
                    $filerecord = array('contextid' => $dstcontext->id, 'component' => 'mod_assignment', 'filearea' => 'onlinejudge_output', 'itemid' => $testcase->id, 'filepath' => '/', 'filename' => basename($testcase->outputfile));
                    $fs->create_file_from_storedfile($filerecord, $file);
                }
            }
        }
    ...
    if ($oldversion < 2011062400) {
        ...
        $sandboxlangs = array('c', 'cpp', 'c_warn2err', 'cpp_warn2err');
        foreach ($sandboxlangs as $old) {
